...
begin
  xQuery:=  'select case when cctt.name =''Open shift'' then ''Öffnung'''+
                    'when cctt.name =''Close shift'' then ''Schliessung'''+
                     'else ''Zwischenbalance'' end as name ,'+
       'coalesce(u.first_name||'' '','')||coalesce(u.last_name,'')casier,'+
       'tc.cd_name, current_date, cct.ct_number,to_char(cct.gaming_date,''YYYY-MM-DD'')as gaming_date,cct.user_explanation,cct.datetime '+
