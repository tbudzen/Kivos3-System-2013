...
begin
  ...
  edAmountOUT.Text:= EmptyStr;
  edAmountIn.Text:= EmptyStr;
end;
  ...
  begin
    ...
    begin
      Add('Select count(*) as id from  tbl_shortcut_user_values where ' +
        ' ((tbl_shortcut_user_values.Amountin is null) and (tbl_shortcut_user_values.AmountOut is null)) '
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            xQuery := 'Insert into tbl_shortcut_user_values(id_shortcut_user, amountout,nr) values (%s,%s,%s)';
            xQuery := format(xQuery, [stringToDataBAse(Fid),
              FloatStrDataBase(xFloat),FloatStrDataBase(-1) ]);
          end
          else
          begin
        ...
        begin
          ...
          if FEdit = false then
          begin
            xQuery := 'Insert into tbl_shortcut_user_values(id_shortcut_user, amountin, nr ) values (%s,%s,%s )';
            xQuery := format(xQuery, [stringToDataBAse(Fid),
              FloatStrDataBase(xFloat),FloatStrDataBase(-1)]);
          end
          else
...
procedure TCashShortCutManager.btnDeleteClick(Sender: TObject);
var
  SelectedRow: integer;
  DC: cxGridDBDataDefinitions.TcxGridDBDataController;
  xQuery: string;
...
begin
  ...
  begin
    xDataSet := TResultHandle.Create;
    xDataSet.Query.Connection.StartTransaction;
    xId := VarToStr(cxGridDBTableView_ListValues.ViewData.Rows[SelectedRow]
      .Values[0]); // String
    xQuery := 'Delete from tbl_shortcut_user_values where id_shortcut_user=' +
      stringToDataBAse(xId, True);
    xDataSet.Add(xQuery);
    xOk := xDataSet.ExecuteSQL;
    if xOk then
    begin
      xDataSet.ClearResult;
      xQuery := 'Delete from tbl_shortcut_user where id=' +
        stringToDataBAse(xId, True);
      xDataSet.Add(xQuery);
      xOk := xDataSet.ExecuteSQL;
    end;
